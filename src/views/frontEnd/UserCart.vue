  ...
  <div class="pageColor pt-3">
    <div class="container">
      <div v-if="cartEmpty" class="text-center">
        ...
        <img
          src="https://github.com/ASIATING/image-repository/blob/main/head%20(7).png?raw=true"
          class="m-auto d-block w-50"
          alt="購物車為空"
        />
      </div>
      <div v-else>
        <h3 class="fw-bold">購物車</h3>
        <v-stepper alt-labels class="stepper-transparent">
          <v-stepper-header>
        ...
        <div class="text-end">
          <button class="btn clearCart" type="button" @click="clearCart()">
            清空購物車
          </button>
        </div>
        <table class="table align-middle mt-3">
          ...
          <tfoot>
            <tr>
              ...
              <td class="text-center">
                {{ cart && cart.total ? cart.total.toLocaleString() : "" }}
              </td>
            </tr>
        ...
        <div class="">
          ...
          <ul>
            <li>
              <strong>付款方式：</strong>我們接受所有主要信用卡和 PayPal
              作為付款方式。訂單處理前需要付款。
            </li>
            <li>
              <strong>運輸方式：</strong
              >我們提供標準和加急運輸選項。運輸費用和交貨時間可能因您的位置和選擇的運輸方式而異。
            </li>
            <li>
              <strong>退換政策：</strong>如果您對購買的商品不完全滿意，您可以在
              30 天內退貨或換貨。請注意，退回的商品必須保持原樣和包裝。
            </li>
            <li>
              <strong>稅費：</strong
              >我們網站上列出的所有價格均為美元，並且不含稅。適用地點的訂單將加收銷售稅。
            </li>
            <li>
              <strong>隱私：</strong
              >我們致力於保護您的隱私。您提供給我們的任何個人信息將僅用於處理您的訂單，並不會與第三方共享。
            </li>
          </ul>
        </div>
      </div>
